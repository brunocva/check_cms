...
// Desempenho (% de acerto) agrupado por tag — cada barra usa a cor da tag.
export function BarTagChart({ data }: { data: PerformanceEntry[] }) {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{ top: 8, right: 16, left: -16, bottom: 24 }}>
        ...
        <Bar dataKey="accuracy" radius={[4, 4, 0, 0]}>
          {data.map((entry) => (
            <Cell key={entry.id} fill={entry.color ?? '#6366f1'} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
